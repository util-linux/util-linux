...
== FILESYSTEM TYPE CONFIGURATION

When the filesystem type is auto-detected (no *-t* option given), *mount* can
use a different kernel driver name than the detected filesystem type. This
allows the filesystem type used for helper program lookup
(**/sbin/mount.**__type__) to remain the original detected name, while the
kernel mount syscall uses the configured driver name.

The mapping is configured via per-filesystem configuration files:

  /etc/mount/fs.d/<type>.conf
  /run/mount/fs.d/<type>.conf
  /usr/lib/mount/fs.d/<type>.conf

where _<type>_ is the detected filesystem type (e.g., _ntfs_). Files in
_/etc_ take priority over _/run_, which takes priority over _/usr/lib_.

The configuration file uses a simple _key_ = _value_ format. Currently
supported keys:

*mounttype*::
Kernel filesystem driver name to use for *mount*(2) or *fsopen*(2) syscalls
instead of the detected filesystem type. For example:
+
  # /etc/mount/fs.d/ntfs.conf
  mounttype = ntfs3
+
This makes *mount* use the _ntfs3_ driver for the kernel syscall while still
searching for **/sbin/mount.ntfs** as the external helper.

The configuration file is only used for auto-detected filesystem types. An
explicit *-t* option or a type specified in _/etc/fstab_ is always used as-is.
...
_/etc/mount/fs.d/<type>.conf_::
per-filesystem type configuration (see *FILESYSTEM TYPE CONFIGURATION*)
