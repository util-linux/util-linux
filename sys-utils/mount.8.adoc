...
For unprivileged users the mount target from _fstab_ is opened before the filesystem is attached and the mount operation uses this file descriptor rather than the path, so the path is not resolved for the second time by the kernel. Symbolic links are not followed in the target path at all, a _fstab_ entry with a symbolic link in the mount point path is unusable for unprivileged users and the mount fails.

WARNING: When the new mount API is not available (Linux before 5.2, or *mount*(8) compiled without the support for it), the legacy *mount*(2) syscall is used. In this case the pinned target is referenced by a _/proc/self/fd/_ path, which requires a mounted _/proc_ filesystem. Without _/proc_ the target path is resolved by the kernel again and the inherent time-of-check-to-time-of-use (TOCTOU) window between the verification and the mount operation remains; if an ancestor directory of the mount target is writable by the unprivileged user, a path component could be swapped to redirect the mount to an unintended location. Administrators should ensure that mount target paths for *user* mounts do not traverse directories writable by unprivileged users.
...
*X-loopdev.direct-io*=*on*|*off*::
Enable or disable direct I/O when setting up a loop device. If a loop device
already exists for the same file, offset and size limit, its direct I/O mode
must match; otherwise the mount fails without changing the existing device.
Without this option, new loop devices use buffered I/O and existing devices
can be reused in either mode. This option does not change an existing mount
or apply when no loop device is needed. See also *losetup*(8) *--direct-io*.
...
+
Note that *mount*(8) ignores this option for unprivileged (non-root) users. In restricted mode symbolic links are not followed in the target path at all, so a mount with a symbolic link in the target fails. Source paths are only canonicalized when starting with _/dev/_. See section "Non-superuser mounts" above for more details.
...
Loop device setup accepts *loop*, *offset*, *sizelimit* and *X-loopdev.direct-io*, corresponding to options of *losetup*(8). (These options can be used in addition to those specific to the filesystem type.)
