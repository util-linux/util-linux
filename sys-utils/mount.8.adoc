...
*mkdir*(2) in octal notation. The default mode is 0755. 
+
This functionality is also supported for non-root users if defined with the 'user'  
option. In this case, the mkdir operation is performed with dropped permissions.
+
The option is also supported as *x-mount.mkdir*, but this notation
is deprecated since v2.30. See also *--mkdir* command line option.
