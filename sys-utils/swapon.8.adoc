...
*--offload-only*::
Activate the area for explicit proactive reclaim, excluding pressure-driven writes and hibernation. Requires kernel support. This policy does not check memory headroom or guarantee reclaim progress; see _Documentation/mm/swap.rst_ in the kernel documentation for reclaim interfaces and limitations.
+
The equivalent *offload-only* option in _/etc/fstab_ or *--options* takes no value. With *--all*, the long option applies to every selected swap area.
+
On devices supporting discard, *--discard* and *--discard=pages* are incompatible with this policy; *--discard=once* is allowed.
...
*offload-only*::
Restrict the area to explicit proactive reclaim. Takes no value. See *--offload-only*.
