...
+
Block all Landlock filesystem accesses (depends on kernel version):
+
...
+
Block TCP connect calls:
+
*setpriv --landlock-access net:connect-tcp*
+
Limit access to abstract unix sockets to those created inside the Landlock domain:
+
*setpriv --landlock-access scope:abstract-unix-socket*
+
...
+
For example, to grant the file read right on files under */boot*:
+
*--landlock-rule path-beneath:read-file:/boot*
+
To allow connect to TCP port 443:
+
