...
If the kernel is compiled with the ­'11 minute mode' option it will be active when the kernel's clock discipline is in a synchronized state. When in this state, bit 6 (the bit that is set in the mask 0x0040) of the kernel's _­time_status_ variable is unset. This value is output as the 'status' line of the *­adjtimex --print* or *­ntptime* commands.
