...
*-a*, *--ascii*::
Convert only non-printable ASCII characters using '^' and M- notation, and disregard printable Unicode characters. In previous versions of *blkid* this was the default behavior. Specifying *--no-encoding* with this option is not possible.
...
*-d*, *--no-encoding*::
Don't encode non-printing characters. Note that the *--output udev* output format uses a different encoding which cannot be disabled.
