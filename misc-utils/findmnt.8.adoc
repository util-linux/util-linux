...
*--local*::
Print only local filesystems, i.e. exclude network filesystems like NFS or CIFS. Note that pseudo filesystems (proc, sysfs, tmpfs, ...) are local too; use *--real* in addition to exclude them. This option is mutually exclusive with *--net*.
...
*--net*::
Print only network filesystems (NFS, CIFS, ...). This option is mutually exclusive with *--local*.
...
*--real*::
Print only real filesystems, i.e. exclude pseudo filesystems. This option is mutually exclusive with *--pseudo*, but it may be combined with *--local* or *--net*, for example *--real --local* prints only local real filesystems.
...
The exit value is 0 if there is something to display, or 1 in case of any
error. An exit value of 1 is also returned when there is nothing to show and a
user-defined filter is used, or if the device path or mount point does not
exist.
