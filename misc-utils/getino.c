...
struct getino_context {
	int		op_flag;	/* controls for what to get the inode, i.e. pidfs, namespace... */
	pid_t		pid;
...
static struct ns_desc ns_info[] = {
	[GETINO_CGROUP_NAMESPACE] = { "cgroup" },
	[GETINO_IPC_NAMESPACE] = { "ipc" },
	[GETINO_NET_NAMESPACE] = { "network" },
	[GETINO_MNT_NAMESPACE] = { "mount" },
	[GETINO_PID_NAMESPACE] = { "pid" },
	[GETINO_TIME_NAMESPACE] = { "time" },
	[GETINO_USER_NAMESPACE] = { "user" },
	[GETINO_UTS_NAMESPACE] = { "uts" },
};
...
static int get_pidfd_ns_ioctl(struct getino_context *ctx) {
	...
	switch (ctx->op_flag) {
		case GETINO_CGROUP_NAMESPACE:
			pidfd_ioctl = PIDFD_GET_CGROUP_NAMESPACE;
			break;
		case GETINO_IPC_NAMESPACE:
			pidfd_ioctl = PIDFD_GET_IPC_NAMESPACE;
			break;
		case GETINO_NET_NAMESPACE:
			pidfd_ioctl = PIDFD_GET_NET_NAMESPACE;
			break;
		case GETINO_MNT_NAMESPACE:
			pidfd_ioctl = PIDFD_GET_MNT_NAMESPACE;
			break;
		case GETINO_PID_NAMESPACE:
			pidfd_ioctl = PIDFD_GET_PID_NAMESPACE;
			break;
		case GETINO_TIME_NAMESPACE:
			pidfd_ioctl = PIDFD_GET_TIME_NAMESPACE;
			break;
		case GETINO_USER_NAMESPACE:
			pidfd_ioctl = PIDFD_GET_USER_NAMESPACE;
			break;
		case GETINO_UTS_NAMESPACE:
			pidfd_ioctl = PIDFD_GET_UTS_NAMESPACE;
			break;
	}
	return pidfd_ioctl;
}

static int pidfd_get_nsfd_or_err(int pidfd, struct getino_context *ctx)
{
	int nsfd, pidfd_ioctl;

	pidfd_ioctl = get_pidfd_ns_ioctl(ctx);
	if (pidfd_ioctl < 0)
		errx(EXIT_FAILURE, _("no appropriate ioctl to act on the desired namespace"));
...
static void print_inode(struct getino_context *ctx)
{
	...
	pidfd = ul_get_valid_pidfd_or_err(ctx->pid, 0);

	if (ctx->op_flag > GETINO_PIDFS) {
		target_fd = pidfd_get_nsfd_or_err(pidfd, ctx);
		close(pidfd);
	} else {
		target_fd = pidfd;
	}
...
static void __attribute__((__noreturn__)) usage(void)
{
	...
	fputsln(_("Prints the unique pidfd or namespace inode number for a given PID."), stdout);

	fputs(USAGE_OPTIONS, stdout);
	fputsln(_(" -p, --print-pid     print the pid and inode (colon-separated) (default: false)"), stdout);
...
{
	int c;
	struct getino_context ctx = { .print_pid = false };
	...
	argv += optind - 1;
	while (*++argv) {
		ctx.pid = strtopid_or_err(*argv, _("invalid PID argument"));
		print_inode(&ctx);
	}
