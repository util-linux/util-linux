...
static void add_to_pairs(struct fdrecv_pair *pairs, int *n_pairs,
			int target_fd, const char *sockspec, int abstract)
{
	int i;

	if (*n_pairs >= FDRECV_PAIRS_MAX) {
		warnx(_("too many SOCKSPEC groups (max %d)"), FDRECV_PAIRS_MAX);
		errtryhelp(EXIT_FAILURE);
	}
	for (i = 0; i < *n_pairs; i++) {
		if (pairs[i].target_fd == target_fd) {
			warnx(_("duplicate target fd %d"), target_fd);
			errtryhelp(EXIT_FAILURE);
		}
	}
	pairs[*n_pairs].target_fd = target_fd;
	pairs[*n_pairs].sockspec = sockspec;
	pairs[*n_pairs].abstract = abstract;
	(*n_pairs)++;
}
	...
	for (i = 0; i < n_pairs; i++) {
		for (j = 0; j < n_pairs; j++) {
			...
			if (recv_fds[j] == pairs[i].target_fd
				...
				int new_fd = dup_fd_cloexec(recv_fds[j], safe_fd);
				if (new_fd < 0)
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "+f:riaoehV", longopts, NULL)) != -1) {
		...
		if (target_fd >= 0) {
			...
			/* -a must appear before the target fd option. */
			if (strcmp(argv[optind], "-a") == 0 ||
			    strcmp(argv[optind], "--abstract") == 0) {
				warnx(_("-a must be placed before the target fd option"));
				errtryhelp(EXIT_FAILURE);
			}

			add_to_pairs(pairs, &n_pairs, target_fd,
				     argv[optind++], pair_abstract);
		}
	}

done_options:
	if (next_abstract) {
		warnx(_("-a must be placed before a target fd option"));
		errtryhelp(EXIT_FAILURE);
	}
	if (!run_argv) {
		if (optind < argc)
