					     ...
					     const char sep)
{
	...
	if (prefix)
		xbuffer_append_string(&ul_buf, prefix);
	for (size_t i = 0; i < data->count; i++) {
		if (i > 0)
			xbuffer_append_char(&ul_buf, sep);
		xbuffer_appendf(&ul_buf, "%d", data->tfds[i]);
	}
...
static char *anon_signalfd_make_mask_string(const char* prefix, uint64_t sigmask)
{
	...
	for (size_t i = 0; i < sizeof(sigmask) * 8; i++) {
		if ((((uint64_t)0x1) << i) & sigmask) {
			...
			if (signame)
				xbuffer_append_string(&ul_buf, signame);
			else
				xbuffer_appendf(&ul_buf, "%d", signum);
		}
	}
					     ...
					     struct anon_inotify_data *data)
{
	...
	list_for_each(i, &data->inodes) {
		...
		struct anon_inotify_inode *inode = list_entry(i,
							      struct anon_inotify_inode,
							      inodes);
		xbuffer_append_string(&ul_buf, ul_buffer_is_empty(&ul_buf) ? prefix : sep);
		...
		xbuffer_append_string(&ul_buf, buf);
	}
