...
to prevent potential pathname resolution hangs. Note that skipping mountpoint
pathname resolution may reduce the accuracy of device and file system identification
(such as `SOURCE` and `DEV` columns) on certain file systems (e.g. btrfs or overlayfs).
