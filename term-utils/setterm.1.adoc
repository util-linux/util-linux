...
*--msglevel* **0**-**8**::
Sets the console logging level for kernel *printk*(9) messages. All messages strictly more important than the specified level will be printed, so a logging level of *8* will print all kernel messages. A logging level of *0* has the same effect as *--msg on* (it enables console logging rather than setting the level). *klogd*(8) may be a more convenient interface to the logging of kernel messages.
+
