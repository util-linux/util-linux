...
256 bytes. The patterns are compiled by the POSIX regex implementation of the
system C library, where complex patterns may require a considerable amount of
memory and stack. The filter is always compiled in the process of the user who
executes the command; use *prlimit*(1) to restrict the resources if necessary.
